...
export function BuyCashStore({ onClose, onPurchaseSuccess }: BuyCashStoreProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative w-full max-w-2xl mx-4 bg-[#0a0a0f] border border-white/[0.08] rounded-xl shadow-2xl shadow-black/60 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 bg-white/[0.02] border-b border-white/[0.06]">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-md bg-[#00ff88]/10 flex items-center justify-center">
        ...
        <div className="flex border-b border-white/[0.06]">
          <button
            ...
            className={`flex-1 py-2.5 text-[10px] font-mono font-bold uppercase tracking-wider transition-colors ${activeTab === "store" ? "text-[#00ff88] border-b-2 border-[#00ff88] bg-[#00ff88]/[0.05]" : "text-white/30 hover:text-white/50"}`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 text-[10px] font-mono font-bold uppercase tracking-wider transition-colors ${activeTab === "history" ? "text-[#00ff88] border-b-2 border-[#00ff88] bg-[#00ff88]/[0.05]" : "text-white/30 hover:text-white/50"}`}
          >
        ...
        <div className="p-5 max-h-[520px] overflow-y-auto">
          {activeTab === "store" && (
            <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {products.map((product, idx) => {
                    ...
                    return (
                      <div
                        key={product.priceId}
                        className={`relative flex flex-col gap-2.5 p-4 rounded-lg border transition-all ${isPopular ? "border-[#00ff88]/40 bg-[#00ff88]/[0.04]" : "border-white/[0.07] bg-white/[0.02] hover:border-white/[0.12]"}`}
                      >
                        {isPopular && (
                          <div className="absolute -top-2.5 left-1/2 -translate-x-1/2">
                            <span className="px-2 py-0.5 text-[8px] font-mono font-bold bg-[#00ff88] text-black rounded-full uppercase tracking-wide">
                              POPULAR
                            </span>
                          </div>
                        )}
                        ...
                        <div className="flex items-center gap-1.5">
                          <Zap className={`w-3 h-3 ${isPopular ? "text-[#00ff88]" : "text-[#00D4FF]"}`} />
                          <span className="text-[9px] font-mono text-white/30 uppercase">Virtual Cash</span>
                        </div>

                        <div>
                          <p className={`text-2xl font-bold font-mono ${isPopular ? "text-[#00ff88]" : "text-white"}`}>
                        ...
                        <button
                          ...
                          className={`w-full h-8 text-[10px] font-mono font-bold rounded-md transition-all disabled:opacity-40 ${isPopular ? "bg-[#00ff88] text-black hover:bg-[#00ff88]/80" : "bg-white/[0.06] text-white hover:bg-white/[0.10] border border-white/[0.08]"}`}
                        >
                          {isLoading ? (
              ...
              {!isSignedIn && (
                <p className="text-center text-[10px] font-mono text-[#FFD700] mt-4">
                  ...
                  {purchases.map((purchase) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[11px] font-mono font-bold text-[#00ff88]">
