    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] font-mono text-muted-foreground/50">OHLCV</span>
          <span className="text-[10px] text-muted-foreground font-mono">{bars.length} bars</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[11px] font-mono font-bold">${lastPrice.toFixed(2)}</span>
          <span className={`text-[10px] font-mono font-bold ${isUp ? "text-[#00B4D8]" : "text-[#ff3366]"}`}>
            {isUp ? "+" : ""}{pctChange}%
          </span>
