...
export function FinishSetupNudgeUI({ incompleteItems, onDismiss }: FinishSetupNudgeUIProps) {
  return (
    <div className="col-span-12 mb-1.5" data-testid="finish-setup-nudge">
      <div className="bg-gradient-to-r from-[#001a10] via-[#00B4D808] to-[#001a10] border border-[#00B4D8]/15 rounded-sm px-3 py-2.5 animate-in slide-in-from-top-2 duration-300">
              ...
              <div className="flex flex-wrap gap-1.5">
                {incompleteItems.map(item => (
                  <Link
                    key={item.id}
                    href={item.href}
                    className="flex items-center gap-1 px-2 py-1 bg-[#00B4D8]/10 border border-[#00B4D8]/20 rounded-sm text-[9px] font-mono text-[#00B4D8] hover:bg-[#00B4D8]/15 transition-colors"
                  >
                    {item.label}
