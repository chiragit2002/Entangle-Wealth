...
export function PaperTradingWidget({ initialSymbol = "", initialPrice, variant = "floating" }: PaperTradingWidgetProps) {
  ...
  if (variant === "floating") {
    if (isMinimized) {
      return (
        <button
          ...
          className="fixed bottom-4 right-4 z-50 flex items-center gap-2 px-3 py-2 bg-card border border-[#00B4D8]/30 rounded-lg shadow-lg shadow-black/50 hover:border-[#00B4D8]/50 transition-colors"
        >
    ...
    return (
      <>
        ...
        <div className="fixed bottom-4 right-4 z-50 w-[320px] bg-background border border-border rounded-lg shadow-2xl shadow-black/20 dark:shadow-black/60 overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 bg-muted border-b border-border cursor-pointer" onClick={() => setIsExpanded(v => !v)}>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-mono text-muted-foreground/50">${portfolio.cashBalance.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
            <span className={`text-[10px] font-mono font-bold ${pnl >= 0 ? 'text-[#00B4D8]' : 'text-[#ff3366]'}`}>
              {pnl >= 0 ? '+' : ''}{pnlPct}%
            </span>
            <button onClick={(e) => { e.stopPropagation(); setIsMinimized(true); }} className="w-11 h-11 flex items-center justify-center text-muted-foreground hover:text-foreground/80 hover:bg-muted/50 rounded transition-colors" aria-label="Minimize paper trading widget">
              <X className="w-3 h-3" />
            </button>
          </div>
        </div>
        ...
        <div className="p-2.5 space-y-2">
          <div className="grid grid-cols-4 gap-1.5">
            <div className="bg-muted/50 rounded-sm p-1.5 text-center">
              <p className="text-[7px] font-mono text-muted-foreground/50">CASH</p>
            ...
            <div className="bg-muted/50 rounded-sm p-1.5 text-center">
              <p className="text-[7px] font-mono text-muted-foreground/50">POSITIONS</p>
            ...
            <div className="bg-muted/50 rounded-sm p-1.5 text-center">
              <p className="text-[7px] font-mono text-muted-foreground/50">TOTAL</p>
              <p className="text-[10px] font-mono font-bold text-foreground">${portfolio.totalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            </div>
            <div className="bg-muted/50 rounded-sm p-1.5 text-center">
              <p className="text-[7px] font-mono text-muted-foreground/50">P&L</p>
          ...
          <div className="flex gap-1">
            <button onClick={() => setTradeSide("buy")} className={`flex-1 py-1 text-[9px] font-mono font-bold rounded-sm transition-colors ${tradeSide === "buy" ? "bg-[#00B4D8]/20 text-[#00B4D8] border border-[#00B4D8]/30" : "bg-muted/50 text-muted-foreground/50 border border-border"}`}>BUY</button>
            <button onClick={() => setTradeSide("sell")} className={`flex-1 py-1 text-[9px] font-mono font-bold rounded-sm transition-colors ${tradeSide === "sell" ? "bg-[#ff3366]/20 text-[#ff3366] border border-[#ff3366]/30" : "bg-muted/50 text-muted-foreground/50 border border-border"}`}>SELL</button>
          </div>

          <div className="grid grid-cols-3 gap-1">
            <input value={tradeSymbol} onChange={e => setTradeSymbol(e.target.value.toUpperCase())} placeholder="AAPL" className="h-7 px-2 text-[10px] font-mono bg-muted/50 border border-border rounded-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-[#00B4D8]/30" />
            <input value={tradeQty} onChange={e => setTradeQty(e.target.value)} placeholder="Qty" type="number" className="h-7 px-2 text-[10px] font-mono bg-muted/50 border border-border rounded-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-[#00B4D8]/30" />
            <input value={tradePrice} onChange={e => setTradePrice(e.target.value)} placeholder="Price" type="number" step="0.01" className="h-7 px-2 text-[10px] font-mono bg-muted/50 border border-border rounded-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-[#00B4D8]/30" />
          </div>

          <div className="flex gap-1">
            <button onClick={executeTrade} disabled={tradeLoading || !isSignedIn} className={`flex-1 h-7 text-[9px] font-mono font-bold rounded-sm transition-colors ${tradeSide === "buy" ? "bg-[#00B4D8] text-black hover:bg-[#00B4D8]/80" : "bg-[#ff3366] text-foreground hover:bg-[#ff3366]/80"} disabled:opacity-40`}>
              {tradeLoading ? "EXECUTING..." : `${tradeSide.toUpperCase()} ORDER`}
            </button>
            <button onClick={resetPortfolio} disabled={!isSignedIn} className="px-2 h-7 text-[9px] font-mono font-bold text-muted-foreground/50 bg-muted/50 border border-border rounded-sm hover:text-muted-foreground transition-colors disabled:opacity-40">
          ...
          {isExpanded && portfolio.positions.length > 0 && (
            <div className="border-t border-border pt-1.5 max-h-[150px] overflow-y-auto">
              <p className="text-[8px] font-mono text-muted-foreground/40 mb-1">OPEN POSITIONS</p>
              {portfolio.positions.map(p => (
                <div key={p.id} className="flex items-center justify-between py-0.5">
                  <span className="text-[10px] font-mono font-bold text-[#00B4D8]">{p.symbol}</span>
                  <span className="text-[9px] font-mono text-muted-foreground/70">{p.quantity} @ ${p.avgCost.toFixed(2)}</span>
                  <span className="text-[9px] font-mono text-muted-foreground">${(p.quantity * p.avgCost).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {isExpanded && portfolio.trades.length > 0 && (
            <div className="border-t border-border pt-1.5 max-h-[120px] overflow-y-auto">
              <p className="text-[8px] font-mono text-muted-foreground/40 mb-1">RECENT TRADES</p>
              {portfolio.trades.slice(0, 5).map(t => (
                <div key={t.id} className="flex items-center justify-between py-0.5">
                  ...
                  <span className="text-[9px] font-mono text-muted-foreground/70">{t.quantity} @ ${t.price.toFixed(2)}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {portfolio.positions.length > 0 && (
            <button onClick={() => setIsExpanded(v => !v)} className="w-full flex items-center justify-center gap-1 py-0.5 text-[8px] font-mono text-muted-foreground/70 hover:text-muted-foreground/70 transition-colors">
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-2 py-1.5 bg-muted/30 border-b border-border border-l-2 border-l-[#00B4D8]">
        <div className="flex items-center gap-1.5">
          <TrendingUp className="w-3 h-3 text-[#00B4D8]" />
          <span className="text-[10px] font-bold uppercase tracking-widest font-mono text-[#00B4D8]">PAPER TRADING</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[9px] font-mono text-muted-foreground/50">${portfolio.cashBalance.toLocaleString(undefined, { maximumFractionDigits: 0 })} CASH</span>
          <span className={`text-[10px] font-mono font-bold ${pnl >= 0 ? 'text-[#00B4D8]' : 'text-[#ff3366]'}`}>
            {pnl >= 0 ? '+' : ''}{pnlPct}%
          </span>
          <button
      ...
      <div className="p-2 space-y-2">
        <div className="grid grid-cols-4 gap-1.5">
          <div className="bg-muted/50 rounded-sm p-2 text-center">
            <p className="text-[8px] font-mono text-muted-foreground/50">CASH</p>
          ...
          <div className="bg-muted/50 rounded-sm p-2 text-center">
            <p className="text-[8px] font-mono text-muted-foreground/50">POSITIONS</p>
          ...
          <div className="bg-muted/50 rounded-sm p-2 text-center">
            <p className="text-[8px] font-mono text-muted-foreground/50">TOTAL</p>
            <p className="text-[11px] font-mono font-bold text-foreground">${portfolio.totalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
          </div>
          <div className="bg-muted/50 rounded-sm p-2 text-center">
            <p className="text-[8px] font-mono text-muted-foreground/50">P&L</p>
        ...
        {portfolio.positions.length > 0 && (
          <div className="border-t border-border pt-1.5">
            ...
            {portfolio.positions.map(p => (
              <div key={p.id} className="flex items-center justify-between py-0.5">
                <span className="text-[10px] font-mono font-bold text-[#00B4D8]">{p.symbol}</span>
                <span className="text-[9px] font-mono text-muted-foreground/70">{p.quantity} shares @ ${p.avgCost.toFixed(2)}</span>
                <span className="text-[9px] font-mono text-muted-foreground">${(p.quantity * p.avgCost).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
              </div>
            ))}
          </div>
