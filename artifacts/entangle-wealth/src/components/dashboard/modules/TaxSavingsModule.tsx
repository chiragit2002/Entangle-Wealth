    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-[#FF8C00]/10 border border-[#FF8C00]/20 flex items-center justify-center">
        ...
        <Link href="/taxgpt" className="flex items-center gap-1 text-[10px] text-[#FF8C00]/70 hover:text-[#FF8C00] transition-colors">
      ...
      <div className="space-y-2">
        {strategies.map((s) => (
          <div key={s.label} className="flex items-start gap-3 py-2 border-b border-white/[0.04] last:border-0">
            <div className={`mt-0.5 w-1.5 h-1.5 rounded-full shrink-0 ${s.priority === "high" ? "bg-[#FF8C00]" : s.priority === "medium" ? "bg-[#FFB800]" : "bg-white/20"}`} />
