...
export function ProgressiveProfileCard({ className = "" }: ProgressiveProfileCardProps) {
  ...
  if (step === "done") {
    return (
      <div className={`bg-[#001a0f] border border-[#FF8C00]/20 rounded-xl p-4 flex items-center gap-3 animate-in fade-in duration-200 ${className}`}>
    ...
    <div className={`bg-[#0a0a14] border border-white/10 rounded-xl p-4 animate-in slide-in-from-top-2 duration-300 ${className}`}>
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          {step === "occupation" ? (
            <Briefcase className="w-4 h-4 text-[#FF8C00]" />
          ) : (
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button
              ...
              disabled={saving}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#FF8C00]/10 border border-[#FF8C00]/20 text-[#FF8C00] text-[11px] font-bold hover:bg-[#FF8C00]/20 transition-colors disabled:opacity-50"
            >
