...
import { Shield, ShieldCheck, Loader2, ExternalLink, User, Check, X, RefreshCw, FileText, Calendar, MapPin, CreditCard, Building2 } from "lucide-react";
...
interface BusinessDocSubmission {
  id: string;
  clerkId: string;
  email: string;
  firstName: string | null;
  ...
  businessDocSubmittedAt: string | null;
  docUrls: string[];
}

const BASE_URL = import.meta.env.BASE_URL?.replace(/\/$/, "") || "";

type Tab = "kyc" | "business";
...
export default function AdminKyc() {
  ...
  const [businessSubmissions, setBusinessSubmissions] = useState<BusinessDocSubmission[]>([]);
  const [loadingBusiness, setLoadingBusiness] = useState(true);
  const [businessActionLoading, setBusinessActionLoading] = useState<string | null>(null);
  ...
  const fetchBusinessSubmissions = async () => {
    setLoadingBusiness(true);
    try {
      const res = await authFetch("/business-docs/admin/submissions", getToken);
      if (res.ok) {
        setBusinessSubmissions(await res.json());
      } else if (res.status === 403) {
        toast({ title: "Access Denied", description: "Admin access required.", variant: "destructive" });
      }
    } catch {
      toast({ title: "Error", description: "Failed to load business document submissions.", variant: "destructive" });
    } finally {
  ...
  const handleKycApprove = async (userId: string) => {
    setKycActionLoading(userId + "-approve");
    try {
  ...
  const handleKycReject = async (userId: string) => {
    setKycActionLoading(userId + "-reject");
    try {
  ...
  const handleBusinessApprove = async (userId: string) => {
    setBusinessActionLoading(userId + "-approve");
    try {
      const res = await authFetch(`/business-docs/approve/${userId}`, getToken, { method: "POST" });
      if (res.ok) {
        setBusinessSubmissions(prev => prev.filter(s => s.id !== userId));
        toast({ title: "Approved", description: "Business documents verified successfully." });
      } else {
        toast({ title: "Error", description: "Failed to approve business documents.", variant: "destructive" });
      }
    } catch {
      toast({ title: "Error", description: "Failed to approve business documents.", variant: "destructive" });
    } finally {
  ...
  const handleBusinessReject = async (userId: string) => {
    setBusinessActionLoading(userId + "-reject");
    try {
      const reason = rejectReason[userId] || "";
      const res = await authFetch(`/business-docs/reject/${userId}`, getToken, {
        method: "POST",
      ...
      if (res.ok) {
        setBusinessSubmissions(prev => prev.filter(s => s.id !== userId));
        toast({ title: "Rejected", description: "Business document submission rejected." });
      } else {
        toast({ title: "Error", description: "Failed to reject business documents.", variant: "destructive" });
      }
    } catch {
      toast({ title: "Error", description: "Failed to reject business documents.", variant: "destructive" });
    } finally {
  ...
  const loading = activeTab === "kyc" ? loadingKyc : loadingBusiness;
          ...
          >
            <span className="flex items-center justify-center gap-2">
              ...
              {submissions.length > 0 && (
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-yellow-500/20 text-yellow-400 text-[10px] font-bold">{submissions.length}</span>
              )}
            </span>
          </button>
          <button
          ...
          >
            <span className="flex items-center justify-center gap-2">
              ...
              {businessSubmissions.length > 0 && (
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-yellow-500/20 text-yellow-400 text-[10px] font-bold">{businessSubmissions.length}</span>
              )}
            </span>
          </button>
        </div>
                  ...
                  <div key={s.id} className="glass-panel p-6 rounded-xl">
                    ...
                    <div className="flex gap-3 pt-3 border-t border-white/10">
                      <Button
                        ...
                        variant="outline"
                        onClick={() => handleKycApprove(s.id)}
                        disabled={kycActionLoading !== null}
                      >
                        {kycActionLoading === s.id + "-approve" ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                        Approve
                      </Button>
                      <Button
                        className="flex-1 bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 gap-2"
                        variant="outline"
                        onClick={() => handleKycReject(s.id)}
                        disabled={kycActionLoading !== null}
                      >
                        {kycActionLoading === s.id + "-reject" ? <Loader2 className="w-4 h-4 animate-spin" /> : <X className="w-4 h-4" />}
                        Reject
                      </Button>
                    </div>
                  </div>
        ...
        {activeTab === "business" && (
          <>
            {loadingBusiness ? (
              ...
              <div className="glass-panel p-12 text-center">
                <ShieldCheck className="w-12 h-12 mx-auto mb-4 text-green-400 opacity-60" />
                <p className="text-lg font-semibold">No pending business submissions</p>
                <p className="text-sm text-muted-foreground mt-1">All business document reviews are up to date.</p>
              </div>
            ) : (
              <div className="space-y-6">
                <p className="text-sm text-muted-foreground">{businessSubmissions.length} pending submission{businessSubmissions.length !== 1 ? "s" : ""}</p>
                  ...
                  <div key={s.id} className="glass-panel p-6 rounded-xl">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold">{s.firstName && s.lastName ? `${s.firstName} ${s.lastName}` : s.email}</p>
                          <p className="text-xs text-muted-foreground">{s.email}</p>
                          {s.businessDocSubmittedAt && (
                            <p className="text-xs text-muted-foreground">Submitted {new Date(s.businessDocSubmittedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                          )}
                        </div>
                      </div>
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">
                        <Building2 className="w-3 h-3" /> Pending Review
                      </span>
                    </div>
                    ...
                    <div className="flex gap-3 pt-3 border-t border-white/10">
                      <Button
                        className="flex-1 bg-green-500/10 text-green-400 hover:bg-green-500/20 border border-green-500/20 gap-2"
                        variant="outline"
                        onClick={() => handleBusinessApprove(s.id)}
                        disabled={businessActionLoading !== null}
                      >
                        {businessActionLoading === s.id + "-approve" ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                        Approve
                      </Button>
                      <Button
                        className="flex-1 bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 gap-2"
                        variant="outline"
                        onClick={() => handleBusinessReject(s.id)}
                        disabled={businessActionLoading !== null}
                      >
                        {businessActionLoading === s.id + "-reject" ? <Loader2 className="w-4 h-4 animate-spin" /> : <X className="w-4 h-4" />}
                        Reject
                      </Button>
                    </div>
                  </div>
