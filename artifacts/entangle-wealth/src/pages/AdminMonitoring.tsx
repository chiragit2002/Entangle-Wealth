  ...
  if (isLoading) {
    return (
      <div className="py-6 flex justify-center">
        <RefreshCw className="w-4 h-4 text-[#00B4D8] animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="border-t border-border bg-muted/30 px-5 py-4 space-y-5">
      ...
      <a
        ...
        className="inline-flex items-center gap-1.5 text-xs text-[#00B4D8] hover:text-[#00B4D8]/80 transition-colors"
      >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {issue.status === "resolved" && (
                <span className="text-[10px] text-[#00B4D8] border border-[#00B4D8]/30 bg-[#00B4D8]/10 px-2 py-0.5 rounded-full">
                  resolved
                </span>
              )}
            </div>
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  ...
  return dir === "asc" ? (
    ...
    <ArrowDown className="w-3.5 h-3.5 text-[#00B4D8]" />
  );
}
