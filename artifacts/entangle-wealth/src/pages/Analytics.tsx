  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        {trend && (
          <div className={`flex items-center gap-1 text-xs font-medium ${trend === "up" ? "text-[#00B4D8]" : trend === "down" ? "text-[#ff3366]" : "text-muted-foreground/70"}`}>
...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    ...
    >
      <h3 className="text-sm font-semibold text-foreground/70 mb-4 uppercase tracking-wider">{title}</h3>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <Star
          key={s}
          className={`w-3 h-3 ${s <= Math.round(rating) ? "text-[#FFB800] fill-[#FFB800]" : "text-muted-foreground/70"}`}
        />
      ))}
    </div>
  );
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Platform health metrics & user intelligence</p>
          </div>
        ...
        >
          ...
          <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Date Range</span>
          ...
          {datePreset === "custom" && (
            <div className="flex items-center gap-2">
              <input
                ...
                className="text-xs px-2 py-1.5 rounded-lg text-foreground focus:outline-none"
          ...
          <>
                    ...
                    return (
                      <div key={item.name}>
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className="text-foreground font-mono">{item.value.toLocaleString()} ({pct}%)</span>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <ChartCard title="Feature Usage (Top 20)">
                ...
                <p className="text-xs text-muted-foreground mt-2 text-center">Click any bar to drill into event details</p>
              </ChartCard>
                  ...
                  <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                    {data.contentByPlatform.map((item, i) => (
                      <div key={`${item.platform}-${item.status}-${i}`} className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-foreground font-mono">{item.count.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="h-80 flex items-center justify-center text-muted-foreground/50 text-sm">
                      ...
                      {data.feedback.recent.map((fb) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-[10px] text-muted-foreground/50 capitalize px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.05)" }}>
                              {fb.category}
                            </span>
                          </div>
                          {fb.comment && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
              <div>
                <h2 className="text-foreground font-semibold text-lg">{formatEventName(drilldownEvent)}</h2>
                {drilldownData && (
              ...
              <button onClick={closeDrilldown} className="text-muted-foreground hover:text-foreground transition-colors p-1">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {drilldownLoading && (
                <div className="flex items-center justify-center h-40 text-muted-foreground/70">
              ...
              {!drilldownLoading && drilldownData && (
                <>
                  {drilldownData.dailyBreakdown.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-foreground/70 uppercase tracking-wider mb-3">Daily Trend</h3>
                      <div className="h-40">
                        <ResponsiveContainer width="100%" height="100%">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-foreground/70 uppercase tracking-wider mb-3">Recent Occurrences</h3>
                    {drilldownData.events.length === 0 ? (
                      <p className="text-muted-foreground/50 text-sm text-center py-8">No events in this date range</p>
                    ) : (
                      <div className="space-y-2">
                        {drilldownData.events.map((evt) => (
                          ...
                          >
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-muted-foreground font-mono">{evt.userId ? `${evt.userId.slice(0, 16)}…` : "Anonymous"}</span>
                              <span className="text-muted-foreground/50">{new Date(evt.createdAt).toLocaleString()}</span>
                            </div>
                      ...
                      <div className="flex items-center justify-between mt-4 text-xs text-muted-foreground/70">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="p-1 rounded disabled:opacity-30 hover:text-foreground transition-colors"
                          >
