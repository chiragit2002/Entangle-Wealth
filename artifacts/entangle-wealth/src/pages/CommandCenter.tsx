      ...
      <div className="flex-1 overflow-auto p-3 space-y-3">
        {final && (
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: "Base Case", val: fmt(final.netWorth), color: "#00D4FF" },
        ...
        {projections.length > 0 && (
          <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full" preserveAspectRatio="xMidYMid meet">
            <polygon points={conePts} fill="rgba(0,212,255,0.07)" />
            ...
            <polyline points={basePts} fill="none" stroke="#00D4FF" strokeWidth="1.5" />
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: "Monthly", key: "monthlyInvestment" as const, min: 0, max: 5000, step: 50, color: "#00D4FF" },
...
function FlashCouncilPanel({ collapsed }: { collapsed: boolean }) {
  ...
  const [agentStatuses] = useState(() =>
    [
      { name: "MACRO-AGENT", color: "#00D4FF", status: "BULLISH", conf: 87 },
      ...
      { name: "MOMENTUM", color: "#00D4FF", status: "BUY", conf: 79 },
    ]
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-auto p-3 space-y-3">
        ...
        >
          <div className="text-[8px] font-mono uppercase tracking-widest mb-1" style={{ color: "rgba(0,212,255,0.6)" }}>
            AI COUNCIL INSIGHT
          </div>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#000810]">
      <div
        ...
        style={{ borderColor: "rgba(0,212,255,0.08)", background: "rgba(0,8,16,0.98)" }}
      >
      ...
      <div className="flex-1 overflow-hidden grid grid-cols-2 gap-0">
        <div className="border-r overflow-auto p-2" style={{ borderColor: "rgba(0,212,255,0.07)" }}>
          ...
          {terminalSystemLog.slice(0, visibleOrders).map((log, i) => (
            ...
            >
              ...
              <span
                ...
                style={{ color: log.level === "WARN" ? "#f5c842" : log.level === "DATA" ? "#a78bfa" : "#00D4FF" }}
              >
                {log.level}
              </span>
      ...
      <div className="border-t p-2 flex-shrink-0" style={{ borderColor: "rgba(0,212,255,0.08)" }}>
        <div ref={scrollRef} className="max-h-16 overflow-y-auto mb-1.5 space-y-0.5">
          {history.map((h, i) => (
            <div key={i} className="text-[9px] font-mono">
              <div style={{ color: "#00D4FF" }}>
                <span style={{ color: "rgba(0,212,255,0.4)" }}>~$ </span>{h.input}
              </div>
        ...
        <form onSubmit={handleCommand} className="flex items-center gap-1.5">
          <span className="text-[9px] font-mono" style={{ color: "rgba(0,212,255,0.4)" }}>~$</span>
          <input
            value={commandInput}
  ...
  if (result && result.journeyData.length > 1) {
    ...
    miniChart = (
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" preserveAspectRatio="xMidYMid meet">
        ...
        <line x1={pad.l} y1={invest} x2={pad.l + pw} y2={invest} stroke="rgba(0,200,248,0.25)" strokeDasharray="4,3" strokeWidth="1" />
...
const PANELS = [
  { id: "monte-carlo", title: "MONTE CARLO CONE", icon: <TrendingUp className="w-3 h-3" />, statusColor: "#00D4FF", statusLabel: "LIVE", href: "/wealth-sim" },
  ...
  const statusBar = (
    ...
    >
      <div className="flex items-center gap-4">
        <span className="text-[9px] font-mono font-bold uppercase tracking-widest" style={{ color: "#00D4FF" }}>
          ...
          <ResizablePanelGroup direction="vertical" onLayout={handleVerticalResize} className="flex-1">
            <ResizablePanel defaultSize={verticalSizes[0]} minSize={20}>
              ...
              >
                ...
                <ResizableHandle className="w-1 bg-white/[0.04] hover:bg-[#00D4FF]/30 transition-colors" />
            ...
            <ResizableHandle className="h-1 bg-white/[0.04] hover:bg-[#00D4FF]/30 transition-colors" />
