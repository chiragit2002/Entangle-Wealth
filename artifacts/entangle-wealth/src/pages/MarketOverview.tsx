...
function heatColor(change: number): string {
  if (change >= 3) return "bg-[#00FF41] text-black";
  if (change >= 1.5) return "bg-[#00FF41]/70 text-black";
  if (change >= 0.5) return "bg-[#00FF41]/40 text-white";
  if (change > 0) return "bg-[#00FF41]/20 text-[#00FF41]";
...
function sectorHeatBg(change: number): string {
  if (change >= 1.5) return "from-[#00FF41]/25 to-[#00FF41]/5";
  if (change >= 0.5) return "from-[#00FF41]/15 to-[#00FF41]/3";
  if (change > 0) return "from-[#00FF41]/8 to-transparent";
        ...
        <div className="container mx-auto px-4 max-w-[1600px]">
          <div className="flex items-center justify-between h-11">
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[9px] font-bold px-2 py-0.5 rounded ${marketStatus === "open" ? "bg-[#00FF41]/10 text-[#00FF41]" : "bg-[#FFB800]/10 text-[#FFB800]"}`}>
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-[10px] text-white/50">A/D: <span className="text-[#00FF41]">{advDecl.adv}</span>/<span className="text-[#ff3366]">{advDecl.decl}</span> ({advDecl.ratio}%)</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {INDICES.map(idx => (
            <div key={idx.ticker} className="bg-[#0a0a16] border border-white/[0.06] rounded-lg p-3">
              <div className="flex items-center justify-between mb-1">
                ...
                {idx.changePercent >= 0 ? <ArrowUpRight className="w-3 h-3 text-[#00FF41]" /> : <ArrowDownRight className="w-3 h-3 text-[#ff3366]" />}
              </div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-[11px] font-mono font-bold ${idx.changePercent >= 0 ? "text-[#00FF41]" : "text-[#ff3366]"}`}>
                  {idx.change >= 0 ? "+" : ""}{Math.abs(idx.change).toFixed(2)}
                </span>
                <span className={`text-[10px] font-mono ${idx.changePercent >= 0 ? "text-[#00FF41]/60" : "text-[#ff3366]/60"}`}>
          ...
          {ECON_INDICATORS.map(e => (
            <div key={e.name} className="bg-[#0a0a16] border border-white/[0.04] rounded-lg px-3 py-2">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                {e.status === "up" ? <ArrowUpRight className="w-3 h-3 text-[#ff3366]" /> : e.status === "down" ? <ArrowDownRight className="w-3 h-3 text-[#00FF41]" /> : <Activity className="w-3 h-3 text-white/40" />}
              </div>
              ...
              <div key={sector.ticker} className={`bg-gradient-to-br ${sectorHeatBg(sector.change)} bg-[#0a0a16] border border-white/[0.04] rounded-xl p-3 hover:border-white/10 transition-all`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[13px] font-black font-mono ${sector.change >= 0 ? "text-[#00FF41]" : "text-[#ff3366]"}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
          <div className="bg-[#0a0a16] border border-white/[0.04] rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/[0.04]">
              <TrendingUp className="w-3.5 h-3.5 text-[#00FF41]/60" />
            ...
            <div>
              {liveGainers.map((s, i) => (
                <div key={s.symbol} className="flex items-center px-4 py-2 border-b border-white/[0.02] hover:bg-white/[0.01] transition-colors">
                  ...
                  <span className="text-[12px] font-mono font-bold text-[#00FF41]">+{s.change.toFixed(2)}%</span>
                </div>
              ))}
            </div>
          </div>
            ...
            {GLOBAL_MARKETS.map(m => (
              <div key={m.name} className="bg-[#0a0a16] border border-white/[0.04] rounded-lg px-3 py-2.5 hover:border-white/10 transition-colors">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {m.change >= 0 ? <ArrowUpRight className="w-2.5 h-2.5 text-[#00FF41]/50" /> : <ArrowDownRight className="w-2.5 h-2.5 text-[#ff3366]/50" />}
                </div>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <span className={`text-[10px] font-mono font-bold ${m.change >= 0 ? "text-[#00FF41]" : "text-[#ff3366]"}`}>
