...
function RadarChart({ sectors }: { sectors: SectorData[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-md mx-auto">
      ...
      <circle cx={center} cy={center} r={3} fill="#00c8f8" opacity="0.8" />
    </svg>
  );
}
        ...
        {!loading && sectors.length > 0 && (
          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="mobile-card text-center">
              ...
              <div className={`text-lg font-bold ${overallMomentum === "Risk-On" ? "text-[#00e676]" : overallMomentum === "Risk-Off" ? "text-[#ff4466]" : "text-[#f5c842]"}`}>
                {overallMomentum}
              </div>
              <div className="text-[9px] text-muted-foreground">Market Regime</div>
            </div>
            <div className="mobile-card text-center">
              <TrendingUp className="w-5 h-5 mx-auto text-[#00e676] mb-1" />
              <div className="text-lg font-bold text-[#00e676]">{bullishCount}</div>
