      ...
      <div className="flex items-start gap-3 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1.5">
            <div>
              <span className="text-xs font-semibold text-foreground/80 font-mono">{agent.name}</span>
              <span className="ml-2 text-xs text-muted-foreground/40">{agent.domain}</span>
            </div>
          ...
          <AnimatePresence>
            {expanded && (
              ...
              >
                <p className="text-xs text-muted-foreground/70 leading-relaxed pb-1">{agent.reasoning}</p>
                <p className="text-xs text-muted-foreground/40 font-mono">KEY: {agent.keyMetric}</p>
              </motion.div>
            )}
          </AnimatePresence>
          {!expanded && (
            <p className="text-xs text-muted-foreground/50 leading-relaxed line-clamp-1">{agent.reasoning}</p>
          )}
        </div>
      </div>
    </motion.div>
    ...
    <FadeIn>
      <div className={`border rounded-2xl p-8 flex flex-col items-center gap-4 ${
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-muted-foreground/70 text-xs max-w-xs">{error.message}</p>
        ...
        <Button size="sm" variant="outline" onClick={onReset} className="border-border text-muted-foreground hover:text-foreground">
          Try Again
        </Button>
      </div>
    </FadeIn>
    ...
    <div className="bg-muted/50 border border-border rounded-2xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono font-semibold text-muted-foreground uppercase tracking-wider">Agent Swarm</span>
        </div>
        <span className="text-xs font-mono text-muted-foreground/50">{current} / {total || "?"} agents complete</span>
      </div>
        ...
        {(total > 0 ? Array.from({ length: total }) : Array.from({ length: 7 })).map((_, i) => (
          <motion.div
            ...
            animate={i < current
              ...
              : { borderColor: "hsl(var(--border))", background: "hsl(var(--muted) / 0.3)", color: "hsl(var(--muted-foreground) / 0.5)" }
            }
          >
            {i < current ? (
...
const CustomRadarTooltip = ({ active, payload }: { active?: boolean; payload?: { value: number; payload: { agent: string } }[] }) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg px-3 py-2 text-xs font-mono">
      <div className="text-muted-foreground mb-0.5">{d.payload.agent}</div>
          ...
          <div className="max-w-[1200px] mx-auto px-4 py-6 space-y-6">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-foreground font-mono">Strategy Evaluator</h1>
                  <p className="text-sm text-muted-foreground/70">Multi-model scoring · Agent-by-agent breakdown</p>
                </div>
              </div>
              {phase !== "idle" && (
                <Button variant="outline" size="sm" onClick={reset} className="border-border text-muted-foreground hover:text-foreground hover:bg-muted/50">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
              ...
              <div className="space-y-5">
                <div className="bg-muted/50 border border-border rounded-2xl p-5 space-y-5">
                  <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest">Configuration</div>

                  <div>
                    <label className="text-xs text-muted-foreground mb-2 block font-mono">Asset</label>
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded text-xs font-mono font-semibold border transition-all ${
                            symbol === s && !customSymbol
                              ...
                              : "bg-muted/30 border-border text-muted-foreground/70 hover:text-foreground/70 hover:border-border"
                          }`}
                    ...
                    <input
                      ...
                      maxLength={10}
                      className="w-full bg-muted/50 border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-cyan-500/40 font-mono transition-colors"
                    />
                  </div>

                  <div>
                    <label className="text-xs text-muted-foreground mb-2 block font-mono">Timeframe</label>
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground mb-2 block font-mono">Market State</label>
                      ...
                      {MARKET_STATES.map(m => (
                        <button
                          ...
                          className={`py-2 px-3 rounded-lg text-xs font-mono font-semibold border transition-all text-left ${
                            marketState !== m.value ? "bg-muted/30 border-border text-muted-foreground/70 hover:text-muted-foreground" : ""
                          }`}
                          style={
                            marketState === m.value
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground mb-2 block font-mono">Evaluation Mode</label>
                    <div className="flex gap-1.5">
                      <button
                  ...
                  {phase !== "idle" && (
                    <div className="text-xs font-mono text-muted-foreground/50 pt-1 space-y-0.5">
                      <span className="text-muted-foreground/40">ASSET</span>{" "}
                      <span className="text-[#00d4ff]/60">{activeSymbol}</span>
                      <span className="mx-2 text-muted-foreground/30">·</span>
                      <span className="text-muted-foreground/40">TF</span>{" "}
              ...
              <div className="space-y-5">
                ...
                <AnimatePresence mode="wait">
                  ...
                  {phase === "idle" && (
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="bg-muted/30 border border-border rounded-2xl flex flex-col items-center justify-center py-20 gap-4"
                    >
                      ...
                      <div className="text-center">
                        <p className="text-muted-foreground/70 text-sm font-mono">Configure and run an evaluation</p>
                        <p className="text-muted-foreground/40 text-xs mt-1">Scores, radar chart, and agent breakdown will appear here</p>
                      </div>
                      <div className="flex gap-2 mt-2">
                        {["M1", "M2", "M3", "M4", "M5", "M6"].map((m) => (
                          <span key={m} className="text-[10px] font-mono text-muted-foreground/30 px-2 py-1 rounded border border-border bg-muted/30">{m}</span>
                        ))}
                      </div>
                    </motion.div>
                  )}
                  ...
                  {phase === "connecting" && (
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="bg-muted/30 border border-border rounded-2xl flex flex-col items-center justify-center py-16 gap-4"
                    >
                      <div className="relative">
                ...
                {(phase === "streaming" || (phase === "done" && evalMode === "deep")) && streamAgents.length > 0 && (
                  <FadeIn>
                    ...
                    <div className="bg-muted/50 border border-border rounded-2xl p-5 mt-5">
                      <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-4">
                        Agent Results
                        {phase === "done" && (
                ...
                {phase === "done" && fastResult && evalMode === "fast" && (
                  <FadeIn>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 flex flex-col items-center">
                        <AnimatedGauge score={fastResult.confidence} label="CONFIDENCE" size={160} />
                        <div className="text-xs text-muted-foreground/50 font-mono mt-2">Confidence Score</div>
                      </div>
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 flex flex-col items-center justify-center gap-3">
                        <div
                        ...
                        <div className="text-xs text-muted-foreground/50 font-mono">Signal</div>
                      ...
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 space-y-3">
                        <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-2">Metrics</div>
                        <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-mono text-muted-foreground">${fastResult.keyLevel}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#00d4ff]/[0.04] border border-[#00d4ff]/15 rounded-2xl p-5 mt-4">
                      ...
                      <p className="text-sm text-muted-foreground leading-relaxed">{fastResult.summary}</p>
                    </div>

                    <div className="text-xs text-muted-foreground/40 font-mono text-center pb-2 mt-2">{fastResult.disclaimer}</div>
                  </FadeIn>
                )}
                  ...
                  <FadeIn>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 flex flex-col items-center">
                        ...
                        <div className="text-xs text-muted-foreground/50 font-mono mt-2">Aggregate Agent Score</div>
                      </div>
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 flex flex-col items-center">
                        <AnimatedGauge score={deepResult.confidenceScore} label="CONFIDENCE" size={160} />
                        <div className="text-xs text-muted-foreground/50 font-mono mt-2">Consensus Confidence</div>
                      </div>
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 space-y-3">
                        <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-2">Result</div>
                        <div
                        ...
                        <div className="space-y-1.5 pt-1">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-mono text-muted-foreground/50">Latency</span>
                            <span className="text-xs font-mono text-muted-foreground">{latencyMs != null ? `${(latencyMs / 1000).toFixed(1)}s` : "—"}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-mono text-muted-foreground/50">Horizon</span>
                            <span className="text-xs font-mono text-muted-foreground">{deepResult.timeHorizon}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-mono text-muted-foreground/50">Agents</span>
                            <span className="text-xs font-mono text-muted-foreground">{deepResult.agents.length}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {radarData.length > 0 && (
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 mt-4">
                        <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-4">Agent Confidence Radar</div>
                        <div className="h-64">
                    ...
                    <div className="bg-muted/50 border border-border rounded-2xl p-5 mt-4">
                      <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-4">Model Score Breakdown</div>
                          ...
                          return (
                            ...
                            >
                              <div className="w-5 text-right">
                                <span className="text-xs font-mono text-muted-foreground/40">M{i + 1}</span>
                              </div>
                              <div className="w-36 flex-shrink-0">
                                <span className="text-xs font-mono text-muted-foreground truncate block" title={agent.name}>{agent.name}</span>
                              </div>
                    ...
                    {deepResult.priceTargets && (
                      <div className="bg-muted/50 border border-border rounded-2xl p-5 mt-4">
                        <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-4">Price Targets</div>
                        <div className="grid grid-cols-3 gap-4">
                          ...
                          <div className="text-center bg-muted/50 border border-border rounded-xl py-4">
                            <div className="text-xs text-muted-foreground/50 font-mono mb-1">Base</div>
                            <div className="text-xl font-bold font-mono text-foreground/70">${deepResult.priceTargets.base}</div>
                          </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                      {deepResult.riskFactors?.length > 0 && (
                        <div className="bg-muted/50 border border-border rounded-2xl p-5">
                          <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-3">Risk Factors</div>
                      ...
                      {deepResult.catalysts?.length > 0 && (
                        <div className="bg-muted/50 border border-border rounded-2xl p-5">
                          <div className="text-xs font-mono font-semibold text-muted-foreground/70 uppercase tracking-widest mb-3">Catalysts</div>
                          <ul className="space-y-1.5">
                    ...
                    {deepResult.flashCouncilSummary && (
                      <div className="bg-[#00d4ff]/[0.04] border border-[#00d4ff]/15 rounded-2xl p-5 mt-4">
                        ...
                        <p className="text-sm text-muted-foreground leading-relaxed">{deepResult.flashCouncilSummary}</p>
                      </div>
                    )}

                    <div className="text-xs text-muted-foreground/40 font-mono text-center pb-2 mt-2">{deepResult.disclaimer}</div>
                  </FadeIn>
                )}
