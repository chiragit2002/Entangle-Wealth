...
const RISK_COLORS = { low: "#FF8C00", medium: "#ffb800", high: "#ff4757" };
              ...
              <div key={strategy.id} className="glass-panel rounded-xl overflow-hidden">
                ...
                >
                  <div className="flex justify-between items-start gap-3 mb-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className="text-[11px] text-[#FF8C00] font-mono">{strategy.code}</span>
                  ...
                  {estimated !== null && estimated > 0 && (
                    <p className="text-[13px] font-mono font-bold text-[#FF8C00] mt-1">
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-white/5 pt-4">
                    <div className="mb-3">
                      <p className="text-[11px] text-[#FF8C00] font-semibold mb-1 uppercase">How It Works</p>
                      <p className="text-[13px] text-white/70 whitespace-pre-line">{strategy.how}</p>
                    </div>
                    <div className="mb-3">
                      <p className="text-[11px] text-[#FF8C00] font-semibold mb-1 uppercase">Who's Eligible</p>
                      <p className="text-[13px] text-white/70">{strategy.eligible}</p>
                    </div>
                    <div className="mb-3">
                      <p className="text-[11px] text-[#FF8C00] font-semibold mb-1 uppercase">Required Documentation</p>
                      <ul className="space-y-1">
                        {strategy.documentation.map((doc, i) => (
                          <li key={i} className="text-[12px] text-white/60 flex items-start gap-2">
                            <span className="text-[#FF8C00] mt-0.5">•</span> {doc}
                          </li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div className="flex gap-2 mt-4">
                      <Button
                        ...
                        className={`gap-1 text-[12px] min-h-[36px] ${
                          inPlan
                            ? "bg-[#FF8C00]/15 text-[#FF8C00] border border-[#FF8C00]/30 hover:bg-[#FF8C00]/25"
                            : "bg-gradient-to-r from-[#FF8C00] to-[#0099cc] text-black font-bold"
                        }`}
