...
const MOBILE_TABS = [
  { key: "terminal", label: "Terminal", icon: TerminalIcon, shortLabel: "TERM" },
  { key: "calc", label: "Calculator", icon: Calculator, shortLabel: "CALC" },
  { key: "signals", label: "Signals", icon: BarChart3, shortLabel: "SIG" },
  { key: "trade", label: "Trading", icon: TrendingUp, shortLabel: "TRADE" },
  { key: "flow", label: "Flow", icon: Shield, shortLabel: "FLOW" },
] as const;

type MobileTab = typeof MOBILE_TABS[number]["key"];

function MobilePanelTabs({ activeTab, onTabChange }: { activeTab: MobileTab; onTabChange: (t: MobileTab) => void }) {
  const activeIndex = MOBILE_TABS.findIndex(t => t.key === activeTab);

  const goPrev = () => {
    if (activeIndex > 0) onTabChange(MOBILE_TABS[activeIndex - 1].key);
  };
  const goNext = () => {
    if (activeIndex < MOBILE_TABS.length - 1) onTabChange(MOBILE_TABS[activeIndex + 1].key);
  };
