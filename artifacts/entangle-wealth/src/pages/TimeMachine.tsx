      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full max-w-4xl mx-auto" preserveAspectRatio="xMidYMid meet">
        <defs>
          <linearGradient id="journey-grad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={isProfit ? "#00e676" : "#ff4466"} stopOpacity="0.3" />
            <stop offset="100%" stopColor={isProfit ? "#00e676" : "#ff4466"} stopOpacity="0.02" />
          </linearGradient>
        </defs>
        {yLines.map((tick, i) => (
          <g key={i}>
        ...
        <line x1={pad.left} y1={investY} x2={pad.left + plotW} y2={investY}
          stroke="rgba(0,200,248,0.3)" strokeDasharray="6,4" strokeWidth="1" />
        <text x={pad.left + plotW + 4} y={investY + 4} fill="rgba(0,200,248,0.5)" fontSize="9" fontFamily="JetBrains Mono">
          Invested
        </text>
        <polygon points={areaPoints} fill="url(#journey-grad)" />
        <polyline points={points.join(" ")} fill="none" stroke={isProfit ? "#00e676" : "#ff4466"} strokeWidth="2" />
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="glass-panel rounded-xl p-4 md:p-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-3">
                ...
                <div className="text-right">
                  <div className={`text-2xl font-black font-mono ${result.totalReturn >= 0 ? "text-[#00e676]" : "text-[#ff4466]"}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatCard icon={DollarSign} label="Invested" value={`$${result.investAmount.toLocaleString()}`}
                sub={`${result.sharesOwned.toFixed(4)} shares @ $${result.startPrice.toFixed(2)}`} color="text-[#00c8f8]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="mobile-card-glow">
                <div className="flex items-center gap-2 mb-2"><Trophy className="w-4 h-4 text-[#00e676]" /><span className="text-xs font-bold text-[#00e676]">Best Day</span></div>
                <div className="font-mono text-lg font-bold text-[#00e676]">+{result.bestDay.pct.toFixed(2)}%</div>
