            ...
            >
              ...
              <div className="p-3 space-y-2" onClick={e => e.stopPropagation()}>
                {dayActivities.length === 0 && (
                  <p className="text-[12px] text-white/20 text-center py-2">No activities planned yet</p>
                )}
                {dayActivities.map(a => (
                  <div key={a.id} className="rounded-lg bg-white/[0.03] border border-white/[0.05] p-3">
                    <div className="flex gap-2 mb-2">
                      ...
                      <input type="text" placeholder="Activity name" value={a.title}
                        ...
                        maxLength={200}
                        className="flex-1 bg-white/[0.05] border border-white/10 rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-primary/30 placeholder:text-white/20 min-w-0" />
                      <button onClick={() => onRemove(a.id)}
                        className="p-2 text-white/20 hover:text-[#ff3366] transition-colors min-w-[36px] min-h-[36px] flex items-center justify-center"
                    ...
                    <div className="flex gap-2 items-center flex-wrap">
                      ...
                      <div className="relative flex items-center">
                        <DollarSign className="absolute left-2 w-3 h-3 text-white/20" />
                      ...
                      <div className="relative flex items-center flex-1 min-w-[120px]">
                        <MapPin className="absolute left-2 w-3 h-3 text-white/20" />
                        <input type="text" placeholder="Location (for map)" value={a.location}
                          ...
                          maxLength={100}
                          className="w-full bg-transparent border-none text-[11px] text-white/40 focus:outline-none placeholder:text-white/15 pl-6 min-w-0" />
                      </div>
                    </div>
                  </div>
                ))}
