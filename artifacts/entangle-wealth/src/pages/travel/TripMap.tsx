...
export default function TripMap({ activities, destinations, highlightDay }: Props) {
  ...
  useEffect(() => {
    ...
    visibleStops.forEach(stop => {
      const color = MARKER_COLORS[stop.type] || "#00B4D8";
    ...
    if (highlightDay !== null) {
      ...
      if (destStops.length > 1) {
        ...
        L.polyline(coords, { color: "#00B4D8", weight: 2, opacity: 0.4, dashArray: "8, 4" })
          .addTo(linesRef.current!);
      }
    }
  ...
  return (
    <div className="relative rounded-sm overflow-hidden border border-[rgba(0,180,216,0.15)]">
      {loading && (
