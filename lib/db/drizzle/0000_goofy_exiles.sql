...
CREATE TABLE "paper_orders" (
	...
	"filled_price" real,
	"reserved_cash" real DEFAULT 0 NOT NULL,
	"created_at" timestamp with time zone DEFAULT now(),
	"updated_at" timestamp with time zone DEFAULT now()
);
--> statement-breakpoint
...
ALTER TABLE "paper_orders" ADD CONSTRAINT "paper_orders_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_paper_orders_user_id" ON "paper_orders" USING btree ("user_id");--> statement-breakpoint
CREATE INDEX "idx_paper_orders_status" ON "paper_orders" USING btree ("status");--> statement-breakpoint
...
CREATE INDEX "idx_balance_transactions_created_at" ON "balance_transactions" USING btree ("created_at");