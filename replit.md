...
- **Quant Strategy Engine (v2)**: Parallel worker-thread execution engine running 1000+ parameterized strategies (RSI, EMA, SMA, MACD, Bollinger, Stochastic, CCI, ROC, OBV, CMF, Williams %R, ADX, and combos) across 60 stocks. Multi-timeframe support: daily (1Day) and intraday (1Hour) bars. O(n) backtest via pre-computed indicator arrays. Worker path: `dist/lib/quantEngine/strategyWorker.mjs`. Market hours check uses US/Eastern DST-aware timezone. Alpaca credentials: `ALPACA_KEY_ID` + `ALPACA_SECRET_KEY`. Engine runs persist to `quant_engine_runs` table. Routes: `/api/quant/signals`, `/api/quant/status`, `/api/quant/run`, `/api/quant/runs`.
...
- **13 Agents**:
  ...
  - `Learning` — three-tier memory (shortTerm rolling 50, episodes 5K buffer, longTerm DB-backed cache); aggregates trade outcomes by strategy×regime into `strategy_regime_insights` table with natural language insight text + confidence scoring; emits `learning_updated` with enriched StrategyInsight payloads
  - `Pattern` — reads LearningAgent's long-term memory to detect failure patterns (negative avg PnL), win/loss streaks (3+ consecutive), and regime vulnerabilities (2+ strategies failing in same regime); subscribes to `learning_updated`; emits `pattern_detected`; scans every 5 min
  - `Recovery` — monitors all agent heartbeats, logs unhealthy agents every 2 minutes
- **CapitalAllocator** (`CapitalAllocator.ts`) — Proportional batch portfolio allocator. Takes all strategy cycle results from `runMulti`, filters for EXECUTE/BUY/SELL decisions meeting min score (30) and confidence (20), ranks by `score × confidence`, assigns proportional weights capped per tier (core 40%, satellite 24%, speculative 12%), applies drawdown penalty (<-25% → 10% cap), normalizes to max exposure (100%). Integrated as post-processing in `Orchestrator.runMulti()`. Each `OrchestratorResult` receives a `portfolioAllocation` field with `{strategyId, symbol, allocation, rawWeight, capped, tier}`.
