...
- **Strategy Event Sourcing**: Append-only event log (`strategy_events`) and point-in-time snapshots (`strategy_snapshots`) for full strategy state reconstruction. Events track every mutation (parameter changes, evaluations, refinements, etc.) with typed payloads. Snapshots capture full strategy state at a version with a pointer to the last processed event, enabling replay from any snapshot. API: GET/POST `/api/strategies/:strategyId/events` (with pagination via `after` and type filtering), GET/POST `/api/strategies/:strategyId/snapshots`, GET `/api/strategies/:strategyId/snapshots/:version` (returns snapshot + events since). DB: `strategy_events` (id, strategy_id, event_type, payload, timestamp), `strategy_snapshots` (id, strategy_id, version, state, last_event_id, created_at). Schema: `lib/db/src/schema/strategyEvents.ts`.
