...
- **Multi-Model Strategy Evaluation Pipeline**: Job-based async strategy evaluation with 6 scoring models (M1-M6: Trend, Mean Reversion, Momentum, Volatility, Volume, Signal Consistency). Includes stress testing across synthetic scenarios (high volatility, low liquidity, range-bound, flash crash, trend reversal), parameter refinement with constraint optimization, strategy rankings, and narrative summaries. API: POST `/api/evaluate`, GET `/api/evaluate/:jobId`, POST `/api/evaluate/:strategyId/stress`, POST `/api/evaluate/:strategyId/refine`, GET `/api/evaluate/rankings`, GET `/api/evaluate/:strategyId/summary`. Frontend: `/eval-pipeline` page with radar charts, stress bar charts, score gauges, and refinement suggestions.
